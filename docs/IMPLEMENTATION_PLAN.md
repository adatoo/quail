...
5. **`quail` CLI** (ADRs D-021, D-022) — "like ollama": a thin client of the running app over a Unix control socket (`Shared/ControlProtocol.swift`, mode 0600), shipped at `Quail.app/Contents/Helpers/quail` (direct build only) and linked into `~/.local/bin` from Settings → General. v1: `start`/`stop`/`restart`/`status`, `list`/`ps`, `run <model> [prompt]` (terminal chat — D-021), `launch <tool>` (Claude Code, Codex, aider, Goose started pointed at Quail from `integrations.json`'s launch recipes — env vars, args and temp files, never the tool's own config), `logs [-f]`, `service enable|disable` (open at login + start the server automatically — wires the long-dormant `autoStartServer`). `bench` ✅ with step 4. v2: `pull`, `rm`, `default`, `ctx`, `config`.
