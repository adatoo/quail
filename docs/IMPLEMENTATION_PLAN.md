...
- [ ] 4. **DeviceInfo + FitEstimator.** Implement the formula in ARCHITECTURE §7; bandwidth table in `catalog.json`. Unit-test with known chips and models (e.g. 8B Q4_K_M on 16 GB → Comfortable at 8K; 32B Q4_K_M on 16 GB → Won't fit). This is also what should start setting `CatalogEntry.contextSize` to something other than `ModelStore`'s fixed 8,192 default.
