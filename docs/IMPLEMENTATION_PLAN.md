...
- [x] 9. **Recommendations + This Mac.** `Recommender` (§7's "curated catalog filtered to Comfortable, sorted by rank, tiered by RAM") plus `Catalog.tier(forMemoryBytes:)`; the Add-model sheet shows a "Recommended for this Mac" section (verdicts fetched via `AppState.loadCatalogVerdicts()`, up to 3 concurrent Hub lookups, cached for the sheet's lifetime) ahead of the full list, and the Models pane's empty state names the top pick instead of a bare "Add model…" button. `AppState.canStart`/`hasServableModel` now refuse Start with no installed GGUF — the menu shows "No model installed" and an "Add model…" shortcut into Settings instead. A new "This Mac" Settings tab surfaces the device facts `FitEstimator` already reads (chip, cores, memory, GPU ceiling, free memory, bandwidth, RAM tier) plus marketing name/model identifier/GPU core count (new `DeviceInfo` fields, read via IOKit) and macOS version — found only by testing: `gpu-core-count`'s IOKit accelerator class name changes every SoC generation (`AGXAcceleratorG16X` on this M4 Pro), so it's found by walking the whole `IOService` plane for the property itself rather than matching a class name.
