...
5. **`quail` CLI** (ADRs D-021, D-022) — "like ollama": a thin client of the running app over a Unix control socket (`Shared/ControlProtocol.swift`, mode 0600), shipped at `Quail.app/Contents/Helpers/quail` (direct build only) and linked into `~/.local/bin` from Settings → General. v1: `start`/`stop`/`restart`/`status`, `list`/`ps`, `chat [model] [prompt]` (terminal chat — D-021, D-025; `run` until 2026-09-24), `launch <tool>` (Claude Code, Codex, aider, Goose started pointed at Quail from `integrations.json`'s launch recipes — env vars, args and temp files, never the tool's own config), `logs [-f]`, `service enable|disable` (open at login + start the server automatically — wires the long-dormant `autoStartServer`). `bench` ✅ with step 4. v2 ✅: `pull <name|owner/repo>[:quant]` (catalog names like ollama's, any HF GGUF repo, progress bar, Ctrl-C cancels and a re-run resumes; `--list` shows the catalog with recommendations and what's installed), `rm`, `default [model|--clear]`, `ctx <model> [size|auto]` (with each size's fit), `config` (endpoint, key masked unless `--show-key`, store, logs, settings), `--version`.
...
| Tag | Result on b11081 |
|---|---|
| `llama3.2:1b-instruct-q4_K_M` | loads, ~123 tok/s |
| `qwen3.6:35b-a3b` + projector | loads (8 s), ~63 tok/s; the projector works (a solid-red image is answered "Red") |
| `gemma3:4b` | fails: `key not found in model: gemma3.attention.layer_norm_rms_epsilon` |
| `gemma4:e4b` | fails: `wrong number of tensors; expected 2131, got 720` |
| `gpt-oss:20b` | fails: `unknown model architecture: 'gptoss'` |

The failures are Ollama's own-engine builds of architectures llama.cpp also supports from other conversions (Quail's catalog already serves `gpt-oss-20b` and Gemma 4 from HF GGUFs). Nothing in the manifest flags which tags are affected, so a registry-backed catalog would list models that then fail to load. Also not tested: `gemma4:31b`. **Remaining work:** a maintainer script `scripts/catalog-candidates` that reads the ModelFit dataset (`runsLocally && openWeights`), drops families already in `catalog.json`, looks each remaining one up on HF (`bartowski`, `unsloth`, `lmstudio-community` GGUF repos), checks the quant file names exist, and prints a paste-ready `Family` stub with `rank`/`role` left blank. It never writes `catalog.json`; a person still picks rows and bumps `revision`. When a revision uses ModelFit, credit it (CC BY 4.0) in `catalog.json`'s `notes` and in About. Rapid-MLX's catalog covers MLX only — see Phase 3 step 13.
...
13. **Rapid-MLX catalog import.** Populate the catalog's `mlx` variants and the MLX recommendations from Rapid-MLX's own catalog instead of curating MLX by hand: `rapid-mlx models --json` (0.14.3: 191 text aliases, each with `alias`, `hf_path`, `size_bytes`, parsers, `is_moe`) and `rapid-mlx recipe --json` (per-Mac "smart"/"fast" picks with `footprint_gb` and `tokens_per_sec`, `schema_version` 1). Apache-2.0. Run against the pinned install from step 5, or read its bundled `aliases.json`; the `hf_path` values are plain HF repo ids, so `HFDownloader` fetches them unchanged. See ARCHITECTURE §10.
