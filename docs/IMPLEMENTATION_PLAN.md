...
- [ ] 4. **ProcessSupervisor.** `Process` with explicit `environment`, stdin closed, stdout/stderr piped to `LogStore`. Wait for port bind by polling `/health` up to 90 s. Restart up to 3 times with 2/4/8 s back-off on unexpected exit. Kill the child on app termination (`applicationWillTerminate` and a `SIGTERM` handler). **Note:** router-mode `llama-server` forks a child process per loaded model — the supervisor must kill the whole process group, not just the router PID.
