...
*As of 2026-09-23. Read [ARCHITECTURE.md](ARCHITECTURE.md) first; this document says what to build in what order, and how to know each step is done.*
...
2. **Try (browser).** "Open Chat in Browser" in the menu opens the runtime's built-in web UI (`Runtime.webUIURL`) — D-001's own "link to them".
3. **Try (native), only if (2) proves insufficient** — a minimal playground window (one conversation, cleared on close; tok/s + TTFT footer), after an ADR (D-020) narrowing D-001.
4. **Benchmark.** A fixed, versioned suite (`quail-bench-1`): load time, prompt processing at 512/4096 tokens, generation over 256 tokens (`ignore_eos`, temperature 0), TTFT; 1 warm-up + 3 runs, median/min/max; exact prompt lengths via `/tokenize`. Each result records hardware (`DeviceInfo`), model (id, repo, quant, size, sha256, params, architecture), engine (runtime + version, ctx, GPU layers, slots, Quail version) and conditions (power, thermal state, other loaded models). Stored locally; Export JSON / Copy as Markdown. Measured speeds replace `FitEstimator` estimates where present (Phase 4 step 4).
5. **`quail` CLI** (ADRs D-021, D-022) — "like ollama": a thin client of the running app over a Unix control socket (`Shared/ControlProtocol.swift`, mode 0600), shipped at `Quail.app/Contents/Helpers/quail` (direct build only) and linked into `~/.local/bin` from Settings → General. v1: `start`/`stop`/`restart`/`status`, `list`/`ps`, `run <model> [prompt]` (terminal chat — D-021), `launch <tool>` (Claude Code, Codex, aider, Goose started pointed at Quail from `integrations.json`'s launch recipes — env vars, args and temp files, never the tool's own config), `logs [-f]`, `service enable|disable` (open at login + start the server automatically — wires the long-dormant `autoStartServer`). v2: `pull`, `rm`, `default`, `ctx`, `config`; `bench` with step 4.
...
4. Measured-speed calibration: store TTFT and tok/s per (model, chip) — delivered by Phase 2b's benchmark; prefer measurements over estimates.
